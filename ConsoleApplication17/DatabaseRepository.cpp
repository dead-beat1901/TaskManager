...
vector<TaskDTO> DatabaseRepository::LoadAllTasks() {
    ...
    const wchar_t* sql =
        L"SELECT t.Id, t.Title, c.Name, t.CategoryId, t.Priority, t.StatusId, "
    ...
    if (SQLExecDirectW(stmt, (SQLWCHAR*)sql, SQL_NTS) == SQL_SUCCESS) {
        while (SQLFetch(stmt) == SQL_SUCCESS) {
            ...
            SQLGetData(stmt, 3, SQL_C_WCHAR, t.CategoryName, sizeof(t.CategoryName), &ind);
            SQLGetData(stmt, 4, SQL_C_LONG, &t.CategoryId, 0, &ind);
            SQLGetData(stmt, 5, SQL_C_LONG, &t.Priority, 0, &ind);
            SQLGetData(stmt, 6, SQL_C_LONG, &t.StatusId, 0, &ind);
            long long ts;
            SQLGetData(stmt, 7, SQL_C_SBIGINT, &ts, 0, &ind);
            t.Deadline = ts;
...
void DatabaseRepository::UpdateTask(const TaskDTO& task) {
    ...
    wchar_t sql[512];
    swprintf_s(sql,
        L"UPDATE Tasks SET Title=N'%ls', Priority=%d, StatusId=%d WHERE Id=%d",
        task.Title, task.Priority, task.StatusId, task.Id);
